...
REPORT="$ROOT/reports/02_pr21_ucagent_original_no_formal.md"
REPORT_DIR="$ROOT/reports/artifacts/02_pr21/original_no_formal"
LOG_DIR="$REPORT_DIR/logs"
UCA_LOG="$LOG_DIR/ucagent_pr21_original_no_formal.log"
MSG_FILE="$LOG_DIR/ucagent_pr21_original_no_formal_messages.jsonl"
TOKEN_REPORT="$REPORT_DIR/token_usage.md"
UCA_TIMEOUT="${UCAGENT_PR21_ORIGINAL_TIMEOUT:-1800}"
...
if [[ "$ucagent_rc" -ne 0 ]]; then
  if [[ -f "$UCA_LOG" ]] && grep -q "ToolComplete:" "$UCA_LOG" && grep -q "ToolExit:" "$UCA_LOG"; then
    echo "[23] UCAgent returned $ucagent_rc after ToolComplete/ToolExit; treating completed mission as success"
  else
...
write_report "STATIC_REVIEW_ONLY_NO_DYNAMIC_REPRO" "本轮使用真实 API 调用了原始 UCAgent 流程，但没有 formal skill，也没有 02 的 Picker/Toffee 动态 DUT。因此 agent 能阅读报告和源码并解释 PR21 现象，不能独立生成新的 counterexample。"
cp "$REPORT" "$WORKSPACE/reports/02_pr21_ucagent_original_no_formal.md"

echo "[23] wrote $REPORT"
