# 场景 5：全 Cache Coverage Plan 与 UCAgent 验证闭环

## 定位

场景 5 不是第五个 bug，也不声称已经完成 NutShell Cache 全功能覆盖率。

它的目标是把完整 Cache 验证所需的 coverage plan、CRV、scoreboard 和 coverage database 先定义成可执行、可检查、可追踪的工程资产，然后用 UCAgent 官方 `RunTestCases` flow 验证这个计划本身是完整且诚实的。
...
| 4. 定义 CRV 场景族 | 人工手写，UCAgent 可生成草稿 | `crv_scenarios` | 已落地为计划 |
| 5. 定义 scoreboard oracle | 人工手写 | `scoreboard_plan` | 已落地为计划 |
| 6. 用 Picker 导出真实 Cache wrapper Python DUT | Picker 操作，人工选择 wrapper 和参数 | `reports/artifacts/*/toffee_dut*` | 02/03/04 已落地，05 不重复导 DUT |
| 7. 用 Toffee/pytest 实现 directed/CRV tests | Toffee 操作，人工补 timing、mock、scoreboard | `unity_test/tests/test_*.py` | 02/03/04 已落地，05 检查计划 |
| 8. 用 UCAgent 跑官方 `RunTestCases` | UCAgent 操作 | `uc_test_report/index.html`、message log | 05 已落地 |
| 9. 对窄窗口高风险点调用 `generic-formal` skill | UCAgent + formal skill | formal report、counterexample log | 01/02/03/04 已落地 |
| 10. 合并 coverage DB，审查 gap，决定是否关闭覆盖 | 人工签核，UCAgent 可生成 gap report | `reports/05_full_cache_coverage_plan.md` | 已落地为计划级闭环 |
...
- 哪些 coverage point 可以关闭，哪些只能标为 partial 或 gap；
- 像 04 中 `L1 resp.ready` 是否必须 eager-ready 这样的协议假设，必须在设计文档或验证计划中明示。

UCAgent 可以帮助生成草稿、执行测试、总结 gap，但最终验证意图和签核需要人工负责。
