...
本项目将形式验证封装为 UCAgent 可调用的通用 skill，并提供一个面向任意 Verilog/SystemVerilog 模块的验证入口。四个内置案例用于证明工具链可用、AI/人工协作有效、并能在 NutShell Cache 上复现真实历史 bug 与发现 latest candidate bug。
...
| 验证覆盖率达标 | 15 | 04 场景 setup coverage `5/5 = 100%`；02/03 场景 coverage `5/5 = 100%`；不声称全 Cache 90% |
